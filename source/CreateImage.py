...
def jalali_converter():
    ...
    output = "{} {} {}, ساعت {}:{}".format(
        time_to_tuple[2], month_persian, time_to_tuple[0], time.hour, time.minute
    )
...
def humanize_number(value, significant_digits=3, strip_trailing_zeros=True):
    ...
    if value == 0:
        ...
        0.001 <= value < 1
    ):  # don't humanize these, because 3.0m can be interpreted as 3 million
    ...
    def create(self):
        try:
            ...
            filedata = filedata.replace(
                "%qTotTran5J%", humanize_number(infoTs["qTotTran5J"])
            )
            ...
            pClosing_percentage = round(
                (1 - infoTs["priceYesterday"] / infoTs["pClosing"]) * 100, 2
            )
            filedata = filedata.replace(
                "%pClosing_percentage%", e2p(str(pClosing_percentage))
            )
            filedata = filedata.replace(
                "closing_pointer",
                str(
                    (
                        (infoTs["pClosing"] - infoTs["priceMin"])
                        / (infoTs["priceMax"] - infoTs["priceMin"])
                        * 100
                    )
                )
                + "%",
            ...
            pDrCotVal_percentage = round(
                (1 - infoTs["priceYesterday"] / infoTs["pDrCotVal"]) * 100, 2
            )
            filedata = filedata.replace(
                "%pDrCotVal_percentage%", e2p(str(pDrCotVal_percentage))
            )
            filedata = filedata.replace(
                "pDrCotVal_pointer",
                str(
                    (
                        (infoTs["pDrCotVal"] - infoTs["priceMin"])
                        / (infoTs["priceMax"] - infoTs["priceMin"])
                        * 100
                    )
                )
            ...
            if infoTs["priceMax"] > infoTs["priceYesterday"]:
                if (1 - infoTs["priceMax"] / up_period) == 0:
                    ...
                    filedata = filedata.replace(
                        "postive_percentage",
                        str((1 - infoTs["priceMax"] / up_period) * 500) + "%",
                    )
            else:
            ...
            if infoTs["priceMin"] < infoTs["priceYesterday"]:
                if (infoTs["priceMin"] / down_period - 1) == 0:
                    ...
                    filedata = filedata.replace(
                        "negative_percentage",
                        str((infoTs["priceMin"] / down_period - 1) * 500) + "%",
                    )
            else:
            ...
            for i in info_tse:
                ...
                for key, value in i.items():
                    ...
                    if key == "zOrdMeDem":
                        tmp = tmp.replace(
                            "|percentMeDem|", str(int(value / cole * 100))
                        )
                    elif key == "zOrdMeOf":
            ...
            filedata = filedata.replace(
                "|buy_I_Percent|",
                e2p(str(round(info_grabbed["buy_I_Volume"] / buy, 2) * 100)),
            )
            filedata = filedata.replace(
                "|sell_I_Percent|",
                e2p(str(round(info_grabbed["sell_I_Volume"] / buy, 2) * 100)),
            )
            filedata = filedata.replace(
                "|buy_N_Percent|",
                e2p(str(round(info_grabbed["buy_N_Volume"] / buy, 2) * 100)),
            )
            filedata = filedata.replace(
                "|sell_N_Percent|",
                e2p(str(round(info_grabbed["sell_N_Volume"] / buy, 2) * 100)),
            )

            CHART_DATA = []
            for date, data in enumerate(
                TSE_OBJ.get_daily_history(code, 200)["closingPriceDaily"]
            ):
                CHART_DATA.append(
                    {
                        "t": date,
            ...
            hti.screenshot(
                html_file=f"html/{code}.html", save_as=f"{code}.png", size=(550, 780)
            )
            return True
