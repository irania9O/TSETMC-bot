...
async def me(client, message):
    try:
        ...
        conn.commit()
    await message.reply(
        "سلام،\nخوش اومدی!\nبرای استفاده از ربات یا از دکمه های زیر استفاده کن یا به طور کامل اسم نماد مورد نظرتو بفرست.",
...
⚖️ کاربر گرامی، چنانچه شما از ربات بمب سورس استفاده نمایید به منزله قبول قوانین است""",
        reply_markup=main_keyboard,
    )
...
async def just_text(client, message):
    ...
    try:
                ...
                [
                    [
                        InlineKeyboardButton(  # Generates a callback query when pressed
                            "پیام های ناظر", callback_data=f"{code}"
                        )
                    ]
                ]
            ),
        ...
        await client.send_message(
            message.from_user.id, "مشکلی رخ داد دوباره امتحان کنید."
        )
...
async def answer(client, callback_query):
    ...
    for data in Tsetmc().supervisor_messages(str(code))["msg"]:
        time_to_str = "{},{},{}".format(
            str(data["dEven"])[0:4], str(data["dEven"])[4:6], str(data["dEven"])[6:8]
        )
